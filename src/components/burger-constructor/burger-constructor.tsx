...
const BurgerConstructor: React.FC = () => {
  ...
  const [, ref] = useDrop({
    ...
    drop: (item: IngredientType) => {
      ...
      if (!item.type) {
        console.error(
          `Ошибка в перемещении ингредиента: недействительный тип ${item}`);
        return;
      }
      switch (item.type) {
        case MAIN_TYPE:
  ...
  return (
    ...
    >
      {bun ? <>
        ...
        <p className="text text_color_inactive text_type_main-default">Пожалуйста,
                                                                       перенесите
                                                                       сюда
                                                                       булку
                                                                       и
