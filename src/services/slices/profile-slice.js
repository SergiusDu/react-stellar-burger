...
export const logoutUser = createAsyncThunk('profile/logoutUser', async (args, {rejectedWithValue}) => {
    const refreshToken = getRefreshTokenFromCookies();
    if (refreshToken) {
        const bodyData = {
            token: refreshToken,
...
export const changeUserData = createAsyncThunk('profile/changeUserData', async (userData, {rejectedWithValue}) => {
    const accessToken = getAccessTokenFromCookies();
    if (accessToken) {
        const authorizationHeader = {
            Authorization: accessToken,
        };
        const response = await fetchAPI(CHANGE_USER_DATA_ENDPOINT, PATCH_METHOD, userData, authorizationHeader);
        return {
...
export const getUserData = createAsyncThunk('profile/getUserData', async (ags, {rejectedWithValue}) => {
    ...
    if (authToken) {
        try {
            const response = await fetchAPI(getUserDataUrl, GET_METHOD, null, {Authorization: authToken});
            return {
                name: response.user.name,
                login: response.user.email,
            };
