...
export const registerUser = createAsyncThunk<RegistrationResponse, TUserData, { rejectValue: string }>(
  'registrationData/register-user', async (
    userCredentials,
    {rejectWithValue},
  ) => {
...
export const registrationSlice = createSlice({
  name: 'registrationData', initialState, reducers: {
    ...
    builder.addCase(registerUser.pending, (state) => {
      ...
      setCookie(
        'accessToken', action.payload.accessToken, ACCESS_TOKEN_LIFETIME);
      setCookie(
        'refreshToken', action.payload.refreshToken, REFRESH_TOKEN_LIFETIME);
      state.password = '';
      state.email = '';
      state.name = '';
