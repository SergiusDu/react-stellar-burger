...
import {
  ...
  fetchIngredients, increaseIngredientAmount,
  ingredientSlice, resetAllIngredientAmount, resetSelectedIngredient,
  selectedIngredient,
  selectIngredients,
  selectIsLoading,
  selectIsModalOpen, setBunCount, setSelectedIngredient,
...
describe('ingredientSlice reducer and actions', () => {
  ...
  it('should handle setSelectedIngredient', () => {
    ...
    const action = {
      type: setSelectedIngredient.type, payload: mockIngredient,
    };
  ...
  it('should handle increaseIngredientAmount', () => {
    ...
    const action = {
      type: increaseIngredientAmount.type, payload: mockIngredient._id,
    };
    const state = ingredientSlice.reducer(initialStateWithIngredient, action);
    const ingredientCount = state.ingredients.find(i => i._id === mockIngredient._id)?.count;
  ...
  it('should handle decreaseIngredientAmount', () => {
    ...
    const initialStateWithIngredient = {
      ...initialState,
      ingredients: [{ ...mockIngredient, count: 2 }],
    };
    const action = {
      type: decreaseIngredientAmount.type, payload: mockIngredientId,
    };
    const state = ingredientSlice.reducer(initialStateWithIngredient, action);
    const ingredientCount = state.ingredients.find(i => i._id === mockIngredientId)?.count;
  ...
  it('should handle setBunCount', () => {
    ...
    const action = { type: setBunCount.type, payload: bunId };
    const state = ingredientSlice.reducer(initialStateWithBuns, action);
    expect(state.ingredients.find(i => i._id === bunId)?.count).toBe(1);
    expect(state.ingredients.find(i => i._id !== bunId && i.type === 'bun')?.count).toBeUndefined();
  });
});
